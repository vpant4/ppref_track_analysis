...
void Eff_particlespecies_strange()
{
  ...
  TH1F *sigmapluseff = (TH1F*) hsigmaplusmatchedpt->Clone("hsigmaplusmatchedpt");
  TH1F *sigmaminuseff = (TH1F*) hsigmaminusmatchedpt->Clone("hsigmaminusmatchedpt");
  TH1F *omegaminuseff = (TH1F*) homegaminusmatchedpt->Clone("homegaminusmatchedpt");
 
  inceff->Divide(htotgenpt);
  papeff->Divide(hpapgenpt);
  pioneff->Divide(hpiongenpt);
  Keff->Divide(hKgenpt);
  sigmapluseff->Divide(hsigmaplusgenpt);
  sigmaminuseff->Divide(hsigmaminusgenpt);
  omegaminuseff->Divide(homegaminusgenpt);
  ...
  //set1DStyle(Keff,4);
  set1DStyle(sigmapluseff,2);
  set1DStyle(sigmaminuseff,3);
  set1DStyle(omegaminuseff,4);

 
  //  inceff->GetXaxis()->SetRangeUser(0.4,200.);
  inceff->GetYaxis()->SetRangeUser(0.,1.);
  inceff->Draw("p hist");
  //papeff->Draw("p hist same");
  //pioneff->Draw("p hist same");
  //Keff->Draw("p hist same");
  sigmapluseff->Draw("p hist same");
  sigmaminuseff->Draw("p hist same");
  omegaminuseff->Draw("p hist same");
  ...
  legend1->AddEntry(sigmapluseff,"#Sigma^{+}","p");
  //legend1->AddEntry(peff,"p","p");
  //legend1->AddEntry(apeff,"#bar{p}","p");
  legend1->AddEntry(sigmaminuseff,"#Sigma^{-}","p");
  legend1->AddEntry(omegaminuseff,"#Omega^{-} ","p");
  ...
  c2->SaveAs("/Users/vipulpant/ppref2024analysis/ppref_track_analysis/Plots/Effparticlespecies_strange.pdf");
