...
//________________
void plotComparison(TCanvas *c, TH1D *h1, TH1D *h2,TH1D *h3, TH1D *hRatio1, TH1D* hRatio2,
...
void Compare_gen_reco() {
    ...
    TFile *f = TFile::Open("/Users/vipulpant/cernbox/ppref2024/pprefMCpthat15_effcorr_unweighted.root","READ");
   

    TH1D *htrkptreco_corr = (TH1D*) f->Get("htrkpt_corr");
    TH1D *htrkptreco_uncorr = (TH1D*) f->Get("htrkpt");
    TH1D *htrkptgen       = (TH1D*) f->Get("hgentrkpt");

    
    set1DStyle(htrkptreco_corr, 0);
    set1DStyle(htrkptgen, 0);
    set1DStyle(htrkptreco_uncorr, 0);
   

    Rescale(htrkptreco_corr);
    Rescale(htrkptgen);
    Rescale(htrkptreco_uncorr);

    
    TH1D *hptRatiocorr_uncorr = dynamic_cast<TH1D*>( htrkptreco_corr->Clone("htrkptrecocorr") );
    TH1D *hptRatioreco_gen = dynamic_cast<TH1D*>( htrkptreco_corr->Clone("htrkptgen") );
    
    
    make1DRatio(hptRatiocorr_uncorr, htrkptreco_uncorr, "corr/uncorr", 2);
    ...
    plotComparison(c1, htrkptreco_corr,htrkptreco_uncorr,htrkptgen,hptRatiocorr_uncorr,hptRatioreco_gen, "Reco Corr forest","Reco uncorr forest","Gen",0.5,1000.,"Track pT");

    c1->SaveAs("../Plots/MCclosuresforest.pdf");
